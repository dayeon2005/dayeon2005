...
## 1. 초기 설정 ##
#################
# st.session_stage["page"] → 현재 사용하고 있는 페이지
#  1.Main Page
#  2. Insert(등록) Page
#  3. Update(수정) Page
...
def update_page():
    ...
    with st.form("update_form"):
        book_isbn = st.text_input("ISBN", value=rows["book_isbn"], disabled=True)
        book_name = st.text_input("도서명", value=rows["book_name"])
        book_writer = st.text_input("저자", value=rows["book_writer"])
        book_publisher = st.text_input("출판사", value=rows["book_publisher"])
        book_price = st.text_input("가격", value=rows["book_price"])
        register_at = st.text_input("등록일자", value=rows["register_at"])
        useyn = st.text_input("사용유무", value=rows["useyn"])
